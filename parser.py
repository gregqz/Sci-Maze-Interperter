...
def get_function(line):
    ...
    if function.lower().startswith('if'): # Conditional
        ...
        if condition == '*':
            ...
            match = re.search(r'(-?[0-9]+\.?[0-9]*)|((?<=")[^"]*(?=")|(?<=\')[^\']*(?=\'))|( @)', function)
            if match.group(1):
        ...
        if condition == '*':
            ...
            function = lambda value: then if Decimal(value) <= Decimal(number) else else_
        elif condition == '==':
            if string:
            ...
            if number is None:
                function = lambda value: then if Decimal(value) != Decimal(run.switchgate) else else_
            else:
                function = lambda value: then if Decimal(number) != Decimal(run.switchgate) else else_
        elif condition == '=@':
            if number is None:
                function = lambda value: then if Decimal(value) == Decimal(run.switchgate) else else_
            else:
        ...
        if operator == '=':
            ...
            int_match = re.search(r'-?[0-9]+\.?[0-9]*', function)
            ...
            if operator == '-=':
                function = lambda value: Decimal(Decimal(value) - Decimal(number))
            elif operator == '+=':
                function = lambda value: Decimal(Decimal(value) + Decimal(number))
            elif operator == '*=':
                function = lambda value: Decimal(Decimal(value) * Decimal(number))
